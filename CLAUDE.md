...
- **coding-cluster frontier-baseline sweep** — OPEN (NEW, carried from the June 24 DeepSeek V4 refresh). The DeepSeek refresh dropped stale "near-parity" framing and re-baselined to the current closed frontier (Opus 4.8 ~88.6%, Fable 5 ~95.0%). Other AI Coding pages may still compare to a stale frontier (Opus 4.6/4.7, GPT-5.x snapshots). Sweep review/claude-code, the coding comparison posts, and best-ai-coding-agents-2026 for outdated frontier baselines and re-anchor to Opus 4.8 / Fable 5.
  ...
  - **STILL OPEN (the 10th page): review/claude-code.html — its own sub-pass.** It is the review *of Claude's coding agent*, built throughout on Opus 4.7 (+ Opus 4.6 for the 1M-context GA claim). Bumping to Opus 4.8 means re-checking the 87.6%→88.6% SWE-bench figure AND running a **score checkpoint** (its 8.0 may move) — bigger than a find-replace, so deliberately deferred. The category/coding.html Claude Code card (still "Opus 4.7 at 87.6%") was left to stay consistent with the review until this sub-pass runs.
